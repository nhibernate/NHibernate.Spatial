...
namespace Tests.NHibernate.Spatial
{
    public abstract class ProjectionsFixture : AbstractFixture
    {
        ...
        protected virtual Type GeometryType => GeometryType;

        [Test]
        public virtual void CountAndUnion()
        {
            var results = _session.CreateCriteria(typeof(County))
            ...
            var expected = Wkt.Read("POLYGON((1 0, 1 1, 1 2, 2 2, 3 2, 3 1, 3 0, 2 0, 1 0))");
            var aggregated = (Geometry) result[1];
        ...
        private static void CountAndUnionByState(IList results)
        {
            ...
            var aggregatedAA = (Geometry) resultAA[2];
            var aggregatedBB = (Geometry) resultBB[2];

            var expectedAA = Wkt.Read("POLYGON((1 0, 1 1, 3 1, 3 0, 1 0))");
            var expectedBB = Wkt.Read("POLYGON((1 1, 1 2, 3 2, 3 1, 1 1))");
